...
export interface HeroDef {
  ...
  attackSkill: SkillId;
  /**
   * 这个角色的护身技。默认**开局没有**，要靠抽牌拿到（除非 passiveAtStart）。
   *
   * 写在角色表上是因为它是四个角色之间唯一不共享的一张牌 —— 主动技和发射技那五张谁都能抽到
   * （见 game/skillLoadout.ts 的 runSkillPool），护身技一人一张，它才是"这个角色练成什么样"
   * 的分岔点。
   */
  passive: SkillId;
  /**
   * 开局就戴着护身技，不用抽。
   *
   * 这是给**开局太弱**的角色补的一块底：一级护身技是一包不大的属性加成（见 data/passives.ts），
   * 抽到它之前那几分钟，角色只有自动攻击技和一双靴子。别的角色撑得住，骑士撑不住 —— 他的
   * 攻击力最低、出手最慢、回旋那个圈只有二十个单位，开局那几波全靠硬扛，而他赖以硬扛的正是
   * 这一张牌。
   *
   * 不是白给：它占掉护身技那唯一一格（见 SkillCategoryRules 的 guard），所以拿到它的人在
   * 那一格上再没有选择；而且它照样要靠抽牌升级，开局带的是一级。
   */
  passiveAtStart?: boolean;
}
