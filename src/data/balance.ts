 ...
 *
 * 0.012 看着极小，但它乘的是**整局的总输出**：一局打出几十万点伤害，一级就是几千点回血。
 * 而且它的回复量自动跟着人堆的密度走 —— 越危险的地方回得越快，而那正是它该有的形状。
 *
 * 上限卡在一局最大血上：回血不能超过上限（和丹同一条规矩）。
 */
export const LIFESTEAL_PER_LEVEL = 0.012;

/**
 * 回春：每一跳回血上限的几成（一秒一跳，见 REGEN_TICK）。再乘这一招的伤害倍率。
 *
 * 一级是 0.1 × 1.2 = 每秒一成二，十秒回满；满级 0.1 × 2.4 = 每秒四分之一，四秒就满。
 * 看着多，但它得提前十秒按下去，而末波站桩挨打每秒掋四百多点 —— 回血追不上掉血，
 * 它只是把你从'还能撑'拉回'还能打'。
 */
export const MEND_HP_PER_TICK = 0.1;

/*
 * 狂暴的三个数。三件事一起发生，缺一件它就不成立。
 *
 * 输出那一项跟着等级长（乘伤害倍率），而掉血和降防不长 —— 升级该把这一招变得更值，
 * 而不是更危险。不然练满之后玩家反而不敢按。
 */
/** 狂暴期间攻击力多少。再乘这一招的伤害倍率：一级 +36%，满级 +72%。 */
export const BERSERK_ATTACK = 0.3;
/** 防御掋多少。不跟等级走。 */
 ...
 */
export const BERSERK_HP_DRAIN = 0.04;
