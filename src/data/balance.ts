...
/** 首领的血每波涨多少。0.78：末波的精锐是第一波的 6.5 倍血（8000 → 51,680）。 */
export const BOSS_HP_PER_WAVE = 0.78;
/**
 * 首领的防御每波涨多少。
 *
 * 比血那条缓得多，而且是故意的：防御在递减公式里的边际收益本来就递减（110 挡掉 52%，
 * 翻到 220 也只挡掉 69%），再陡一点就会滑向"打不动"。它的活是把玩家那堆乘算加成削掉一半，
 * 不是把首领变成免疫。
 */
export const BOSS_DEFENSE_PER_WAVE = 0.13;
/** 首领的攻击每波涨多少。比杂兵那条（0.26）缓，因为他的基数本来就高一截。 */
export const BOSS_ATTACK_PER_WAVE = 0.22;
 ...
 * 的那个兵（骑射 35）再留一点点余量。换句话说：练到末波，敌人也不会比游戏一直以来最快
 * 的那个兵更快。
 *
 ...
 *
 * 量法：无敌的玩家一直走位、开自动攻击，把当前可选地图空跑完整一局，记下每一波打完时收到的
 * 灵石（tools 之外的一次性脚本）。结果在"满配那一波"上高度一致：
 *
 ...
 * 封到 5 是因为十项属性 × 5 = 50，比一局能抽的张数还多一截：封顶只掘掉“全堆一项”，
 * 不会把牌库提前抽干。
 */
