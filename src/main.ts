 ...
 * RGBA 加一张同样大的显存纹理。所以按地图存着 —— 玩家在备战界面上来回切图不该每次
 * 重建，进图之后再退回来也不该。
 *
...
/**
 * 四个主动键位（Q/W/E/R）这一帧按着没有。数组复用，别长期持有。
 *
 * 按住型的招（法相）读它；疾走不在这四格里，它单独走 sprintHeld。
 */
 ...
 * 所以这里不再算准星角度。
 *
 * 走有两条路，说的是同一件事：方向键，或者按住左键朝光标走。**键盘优先** —— 按下方向键的
...
function applyEliteBossHealthBars(on: boolean): void {
  profile.setEliteBossHealthBarsVisible(on);
  scene.eliteBossHealthBarsVisible = on;
 ...
 *   升级牌   把一个已经在用的招升一级。
 *
 * 一局是从"一个自动攻击技 + Shift 上的靴子"开始的，一套配置就是这样一张一张堆出来的。跨局带得
