...
/** 池子容量。按"一次回旋杀一百人、每人溅十片"估的上界的一小半 —— 满了就丢。 */
const CAPACITY = 1200;
...
export class Debris {
  ...
  burst(x: number, y: number, dirX: number, dirY: number, power: number, palette: CharacterPalette): void {
    const blood = Math.round(4 + 5 * power);
    const shards = Math.round(power >= 2 ? 3 + 3 * (power - 1) : 0);
  ...
  private emit(
    ...
    if (kind === KIND_BLOOD) {
      ...
      this.size[i] = 0.8 + Math.random() * 0.9;
  ...
  draw(
    ...
    for (let i = 0; i < this.count; i++) {
      ...
      shapes.rect(v2(sx, sy), s, s, 0, rgba(c.r, c.g, c.b, alpha), depthOf(this.y[i]) + 0.4);
    }
  }
}
