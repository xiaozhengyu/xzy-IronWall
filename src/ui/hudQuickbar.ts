 ...
 *
 * Q/W/E/R 原来各硬写着一张图（横扫、铁布衫、天地法相），那是塑界面时的占位。现在开局四个主动
 * 槽本来就是空的，那几张图永远不是玩家真正装着的招，只会让这些格子在第一帧里长成不属于它们
 * 的样子。四格现在都一样：空着，抽到才长出图来。
 *
 * 最后那一格是疾走，钉死的（见 SPRINT_SKILL）。它不在那四个主动槽里，但仍然是一招、仍然扣蓝、
 * 仍然有冷却，所以它照样要在这条栏上占一格，和别的招同一个样子。
 */
...
export class HudQuickbar {
  ...
  setSkillCooldown(index: number, remaining: number, total: number, countdown = true): void {
    ...
    // 进度量化到 1/128 再写。
    //
    // 这个方法每帧被调五次，而写自定义属性不管值变没变都会让这一格的样式失效。一个六秒的
   ...
   * 药和符，而他手上什么都没有。所以没有就不画，和 Q/W/E/R 四个空技能槽一个样子：只剩一个
   * 空框，捡到第一件才长出图标来。
   */
