 ...
 */
/**
 * 每个技能一张图，**会出现在牌里的那十个互不重样**。
 *
 * 原来有三处共用：金钟罩和开天同一张、天地法相和穿云箭同一张、铁布衫和磐石同一张。于是
 * 三选一里明明是三件不同的东西，牌面上却摆着两张一模一样的图 —— 玩家读牌先看图，第一反应
 * 就是"这一轮出重复了"。符箓图一共十张，正好够那十个一人一张。
 *
 * 剩下三个借 HUD 那套图标：疾走本来就不会出现在牌里（它不进牌库），锋锐和疾锋则和某张属性
